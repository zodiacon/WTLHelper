...
COLORREF ColorHelper::Lighten(COLORREF color, int amount) {
    return RGB(
        (BYTE)std::min(255, GetRValue(color) + 255 * amount / 100),
        (BYTE)std::min(255, GetGValue(color) + 255 * amount / 100),
        (BYTE)std::min(255, GetBValue(color) + 255 * amount / 100));
}

COLORREF ColorHelper::Darken(COLORREF color, int amount) {
    return RGB(
        (BYTE)std::max(0, GetRValue(color) - 255 * amount / 100),
        (BYTE)std::max(0, GetGValue(color) - 255 * amount / 100),
        (BYTE)std::max(0, GetBValue(color) - 255 * amount / 100));
}
