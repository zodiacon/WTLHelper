...
public:
	...
	BOOL SetSubItem(HTREEITEM hItem, const LPTLVITEM pItem) {
		...
		ATLASSERT(pItemT);
		if (pItemT == nullptr) return FALSE;
	...
	COLORREF GetSubItemColor(HTREEITEM hItem, int nSubItem, COLORREF* pBackColor) {
		...
		ATLASSERT(hItem);
		LPTLVITEM pItem = _GetSubItem(hItem, nSubItem);
		if (pItem == nullptr) return CLR_NONE;
		if (pBackColor) {
	...
	LPTLVITEM _GetSubItem(HTREEITEM hItem, int iSubItem) {
		ATLASSERT(hItem);
		if (iSubItem<0 || iSubItem>m_ctrlHeader.GetItemCount()) return nullptr;
		tMapItem* pVal = m_mapItems.Lookup(hItem);
		ATLASSERT(pVal);
		ATLASSERT(iSubItem < pVal->GetSize());
		if (pVal == nullptr) return nullptr;
		if (iSubItem >= pVal->GetSize()) return nullptr;
	...
	void _CalcColumnSizes() {
		...
		if (m_cxHeader - cxClient >= 0) {
			...
			if (si.nPos) {          // Scroll position is non-zero. code is stolen from OnHScroll()
				...
				m_ctrlHeader.SetWindowPos(nullptr,
					-si.nPos,
					0,
	...
	LPARAM OnHScroll(UINT /*uMsg*/, WPARAM wParam, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
		...
		RECT rcTree;
		m_ctrlTree.GetClientRect(&rcTree);
		m_ctrlTree.ScrollWindowEx(nPrevPos - nCurPos, 0, nullptr, nullptr, nullptr, nullptr, SW_INVALIDATE);
		...
		if (rcTree.right - rcTree.left != 0) {
			...
			m_ctrlHeader.SetWindowPos(nullptr, m_nOffset, 0, abs(m_nOffset) + iVSWidth + rcTree.right - rcTree.left, rcHeader.bottom - rcHeader.top, SWP_SHOWWINDOW);
		}
	...
	LRESULT OnTreeItemInsert(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/) {
		...
		if (hItem == nullptr) return (LRESULT)hItem;
	...
	LRESULT OnTreeItemExpanded(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& bHandled) {
		...
		if (m_ctrlHeader.GetItemCount() > 0) {
			int cx = 0;
			HTREEITEM hItem = m_ctrlTree.GetNextItem(nullptr, TVGN_FIRSTVISIBLE);
			while (hItem != nullptr) {
				RECT rc = { 0 };
	...
	LRESULT OnHeaderItemChanged(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& bHandled) {
		...
		BOOL bDummy;
		OnTreeItemExpanded(0, nullptr, bDummy);
		// Recalc column sizes
		_CalcColumnSizes();
	...
	DWORD OnPrePaint(int /*idCtrl*/, LPNMCUSTOMDRAW lpNMCustomDraw) {
		...
		::SetViewportOrgEx(lpNMCustomDraw->hdc, m_nOffset, 0, nullptr);
